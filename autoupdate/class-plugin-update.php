...
if ( ! defined( 'ABSPATH' ) || class_exists( 'WPLMS_Course_Custom_Nav_Plugin_Class_Update' ) )
	return;
 ...
 *
 */
class WPLMS_Course_Custom_Nav_Plugin_Class_Update{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'wplms-ccn' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'wplms-ccn' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'wplms-ccn' ), wp_remote_retrieve_body( $request ) );
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		if ( isset( $hook_extra ) ){
			...
			if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] == $plugin_base ){
				...
				/* Update message */
				$fail = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'wplms-ccn' );
				$success = __( 'Plugin reactivated successfully. ', 'wplms-ccn' );
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . __( ' Plugin Updates', 'wplms-ccn' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. __( 'Username: ', 'wplms-ccn' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Email: ', 'wplms-ccn' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Key: ', 'wplms-ccn' ) . '<code>' . $key . '</code></p>';
			}


			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				_e( '<p>Your plugin update is <span style="color:green">active</span></p>', 'wplms-ccn' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'wplms-ccn' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Edit Key', 'wplms-ccn' ) . '</a></p>';
			}
			/* else */
			else{
				_e( '<p>Unable to validate update activation.</p>', 'wplms-ccn' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Try again', 'wplms-ccn' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Add Key', 'wplms-ccn' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php _e( 'User name', 'wplms-ccn' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Email', 'wplms-ccn' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Themeforest User name ( for Auto-Updates )', 'wplms-ccn' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
